...
c.mainloop()"""
#19 -> bolo na ulohu
#19 riesenie len so string
import tkinter
c = tkinter.Canvas()
c.pack()
def stvorce(velkost, farby):
    x, y = 30, 100
    for i in range(farby.count(" ")):
        medz = farby.find(" ")
        c.create_rectangle(x, y, x+velkost, y-velkost, fill=f"{farby[:medz]}")
        farby = farby[medz+1:]
        x+=velkost+5
    c.create_rectangle(x, y, x+velkost, y-velkost, fill=farby)
